...
class MooreMachine:
    ...
    def run(self):
        global time_until_stop ####
        print("inputs:\n(a) 11001101\n(b) 10100111\n") ####
        self.outreg='x'*(time_until_stop-1) ####
        yield self.env.timeout(0.5)
        while True:
            self.outreg = str(self.outputs)[-1] + self.outreg[:-1] ####
            print(f"Time: {self.env.now}, State: {self.ps:03b}, Output: {self.outputs:03b}, Output Reg Reading: {self.outreg} ") ####
